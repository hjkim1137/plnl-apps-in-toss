...
├── App.tsx               # 환경 가드 + 탭 셸  ← 화면 상세는 인정(A)
├── screens/              # 오늘 / 월간 현황 탭  ← 인정(A) 작업 영역 (현재 플레이스홀더)
├── hooks/
│   └── usePlnl.ts        # ★ 모든 상태·액션을 노출하는 핸드오프 훅 (D→A)
└── lib/                  # ★ 개발자(현정/D) 로직 레이어 — 화면 무관, 순수/테스트 가능
    ...
    ├── content.ts        # 카피·등급명·보상값 ← 선민(B)/인정(A) 조정 영역
...
- **D(현정)**: `src/lib/*` + `src/hooks/usePlnl.ts` + 스캐폴딩 + 인증/광고/동기화 + 스키마/백엔드.
- **A(인정)**: `src/screens/*` 화면을 `usePlnl()` 가 주는 값/액션으로 TDS 컴포넌트로 구현.
- **B(선민)**: `src/lib/content.ts` 의 카피·등급명·보상값·표창장 문구 정제.

A 는 로직을 몰라도 됩니다 — `const plnl = usePlnl()` 한 줄이면 `plnl.today.rate`, `plnl.actions.checkIn('done')` 처럼 전부 꺼내 씁니다.
