...
// ────────────────────────────────────────────────────────
section("K. 주간 목표 도장판 (weeklyStamp.ts)");
...
check("K-1 2월(lead=0,28일) → 4주", febResult.weeks.length, 4);
check("K-1 weeklyGoal=ceil(8/4)=2", febResult.weeklyGoal, 2);

// 6월(lead=1, 30일) → 5주
const jun = makeStampCells(1, 30);
const junResult = weeklyStampData(jun, 12);
check("K-2 6월(lead=1,30일) → 5주", junResult.weeks.length, 5);
check("K-2 weeklyGoal=ceil(12/5)=3", junResult.weeklyGoal, 3);

// lead=6(토요일 시작) → 첫 행 1일 + 마지막 행 1일(day30) = 6주
const satStart = makeStampCells(6, 30);
check("K-3 lead=6(30일) → 6주", weeklyStampData(satStart, 10).weeks.length, 6);

// ── K-4~7: 도장 달성 판정 ─────────────────────────────────
// 1주차: done=3, goal=3 → earned
const exactGoal = makeStampCells(0, 28, new Set([1, 2, 3]));
check("K-4 done=goal → earned", weeklyStampData(exactGoal, 8).weeks[0].earned, true);
...
check("K-5 done<goal → not earned", oneShortResult.weeks[0].earned, false);
check("K-5 done=2 표시", oneShortResult.weeks[0].done, 2);

// 1주차: done=7(만석), goal=3 → earned
const fullWeek = makeStampCells(0, 28, new Set([1, 2, 3, 4, 5, 6, 7]));
check("K-6 done=7>goal → earned", weeklyStampData(fullWeek, 8).weeks[0].earned, true);

// missed 는 카운트 안 함 — 7일 모두 missed, done=0 → not earned
const allMissed = makeStampCells(0, 28, new Set(), new Set([1, 2, 3, 4, 5, 6, 7]));
check("K-7 missed only → done=0, not earned", weeklyStampData(allMissed, 8).weeks[0].done, 0);
...
check("K-8 target=1 → weeklyGoal=1", minResult.weeklyGoal, 1);
check("K-8 done=1 → earned", minResult.weeks[0].earned, true);
...
check("K-9 첫주 1일+done=1, goal=2 → not earned", satResult.weeks[0].earned, false);
check("K-9 첫주 done=1 집계", satResult.weeks[0].done, 1);
...
check("K-10b target=0 → weeklyGoal=1(방어값)", emptyResult.weeklyGoal, 1);
check("K-10b done=0 < 1 → not earned", emptyResult.weeks[0].earned, false);
...
check("K-10 1주 earned", mixedResult.weeks[0].earned, true);
check("K-10 2주 not earned(done=1)", mixedResult.weeks[1].earned, false);
check("K-10 3주 earned(done=2=goal)", mixedResult.weeks[2].earned, true);
check("K-10 4주 not earned(done=0)", mixedResult.weeks[3].earned, false);
