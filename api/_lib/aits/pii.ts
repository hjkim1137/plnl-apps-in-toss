// 토스 login-me 가 내려주는 암호화된 PII 복호화 — Vercel 서버리스 함수용(co-located).
// 원본: docs/backend/lib-aits-pii.ts (Next.js 레포 reference).
//
// 알고리즘: AES-256-GCM
// - 키: 콘솔 이메일로 받은 base64 인코딩된 256-bit 키 (TOSS_AITS_DECRYPT_KEY)
// - IV: 암호문 앞 12바이트
// - AAD: 콘솔 이메일로 받은 문자열 (TOSS_AITS_DECRYPT_AAD, 보통 "TOSS")
...
// 키/AAD 는 모듈 상수라 매 복호화마다 다시 디코딩할 필요 없이 1회만 만들어 재사용한다.
let _key: Buffer | null = null;
let _aad: Buffer | null = null;
...
export function decryptPII(encryptedBase64: string): string {
  ...
  const key = (_key ??= Buffer.from(KEY_B64, "base64"));
  const decipher = crypto.createDecipheriv("aes-256-gcm", key, iv);
  decipher.setAAD((_aad ??= Buffer.from(AAD)));
