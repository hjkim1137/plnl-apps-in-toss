...
// ── 스트릭 상태 팝업 카피 — 기획 §2·§3 (StreakPopup) ───────────────────────
// 코드 하드코딩 금지 — 팝업 문구의 단일 출처. \n 은 화면에서 줄바꿈으로 렌더.
// 팝업은 딱 2종: milestone(3·7·14·30일 달성 → 보상 수령) · broken(끊김 → 재시작).
export const STREAK_POPUP_COPY = {
  /** 상단 라벨 — "연속 인증 N일차". */
  label: (streak: number) => `연속 인증 ${streak}일차`,
  /** 마일스톤 달성 — 완료 후 뜨므로 "보상 도착" 톤. */
  milestone: {
    title: "출석 보상이\n도착했어요 🎉",
    /** 이모지는 화면에서 분리 렌더(간격 제어). */
    cta: "출석 보상 받기",
  },
  /** 끊긴 뒤 재시작 격려(당근 '동네걷기' 스타일). 라벨은 label(1)="연속 인증 1일차". */
  broken: {
    title: "다시 1일차부터\n시작해요 💪",
    cta: "확인",
  },
